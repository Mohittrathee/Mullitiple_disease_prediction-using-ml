...
with open('D:/workspace/MachIne Learning/loaded_models/Diabetes_prediction_raw (1).sav', 'rb') as file:
    diabetes_model = pickle.load(file)

with open('D:/workspace/MachIne Learning/loaded_models/heart_disease_raw.sav', 'rb') as file:
    heart_disease_model = pickle.load(file)


with open("D:/workspace/MachIne Learning/loaded_models/parkinsons_data_raw.sav", 'rb') as file:
...
if (selected=='Parkinson Disease Prediction'):
    ...
    with col1:
        NHR=st.text_input('Two measures of ratio of noise to tonal components in the voice,NHR')
        
    with col2:
        HNR=st.text_input('Two measures of ratio of noise to tonal components in the voice,HNR')
    
    with col3:
        RPDE=st.text_input('RPDE')
        
    with col1:
        DFA=st.text_input('Signal fractal scaling exponent,DFA')
    ...
    with col1:
        D2=st.text_input('Two nonlinear dynamical complexity measures,D2')
    ...
    if st.button('Test Results'):
        Parkinson_prediction=parkinson_model.predict([[name,fo,fhi,flo,Jitter_percent,Jitter_abs,RAP,PPQ,DDP,Shimmer,Shimmer_dB,Shimmer_APQ3,Shimmer_APQ5,MDV_APQ,Shimmer_DDA,NHR,HNR,RPDE,DFA,spread1,spread2,D2,PPE]])
    if (Parkinson_prediction[0]==1):
        Parkinson_disease_diagnosis="The person has heart disease"
    else:
        Parkinson_disease_diagnosis="The person has no heart disease"
